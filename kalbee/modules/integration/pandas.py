...
def filter_dataframe(
    ...
    Returns:
        pandas DataFrame with filtered state estimates.

    Example:
        >>> import pandas as pd
        >>> from kalbee import KalmanFilter, filter_dataframe
        >>> df = pd.DataFrame({"t": [0, 1, 2, 3], "x": [1.0, 2.1, 2.9, 4.0]})
    ...
    try:
        ...
        raise ImportError(
            "pandas is required for filter_dataframe. "
    ...
    measurements = df[measurement_columns].to_numpy()
    ...
    if timestamp_column is not None and timestamp_column in df.columns:
        result_data[timestamp_column] = df[timestamp_column].values
...
def filter_series(
    ...
    """
    Run a Kalman filter on a single pandas Series (1D measurement).
    ...
    Returns:
        pandas Series with filtered state estimates (first state dimension).

    Example:
        >>> import pandas as pd
        >>> from kalbee import KalmanFilter, filter_series
        >>> s = pd.Series([1.0, 2.1, 2.9, 4.0], name="x")
        >>> kf = KalmanFilter(state, cov, F, Q, H, R)
    ...
    try:
        ...
        raise ImportError(
            "pandas is required for filter_series. "
            "Install it with: pip install kalbee[pandas]"
        )
    ...
    return pd.Series(state_history, name=f"filtered_{series.name}")
